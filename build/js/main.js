document.addEventListener('DOMContentLoaded', () => {
  ...
  (function () {
    ...
    function resetOverlayIfNeeded() {
      const html = document.documentElement;

      // Если на html нет popup-open, значит по логике стек пуст.
      // Делаем оверлей максимально скрытым и убираем клики.
      if (!html.classList.contains('popup-open')) {
        if (overlayBase) {
          overlayBase.style.transition = '';
